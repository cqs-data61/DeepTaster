...
for size in [100,500,1000,2500]:
    ...
    torch.save(model,  opt.output+'/'+opt.architecture+str(size)+'_10.pt')

    model = train_model(model, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=10)
    torch.save(model,  opt.output+'/'+opt.architecture+str(size)+'_20.pt')

    model = train_model(model, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=10)
    torch.save(model,  opt.output+'/'+opt.architecture+str(size)+'_30.pt')

    model = train_model(model, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=10)
    torch.save(model,  opt.output+'/'+opt.architecture+str(size)+'_40.pt')
